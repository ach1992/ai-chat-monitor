...
function normalizeContinuationText(value: string): string {
  ...
  if (normalized.length === 0 || normalized.length > 200) {
    throw new Error("Continuation text must be between 1 and 200 characters.");
  }
  return normalized;
}
...
function normalizeChatPolicy(policy: ChatAutomationPolicy): ChatAutomationPolicy {
  if (typeof policy.conversationId !== "string" || policy.conversationId.length < 4 || policy.conversationId.length > 200) {
...
function normalizeState(state: AutomationPolicyState): AutomationPolicyState {
  if (state.version !== 1 || !Number.isInteger(state.revision) || state.revision < 1 || typeof state.emergencyPaused !== "boolean") {
    throw new Error("Automation policy state is invalid.");
  }
...
function cloneState(state: AutomationPolicyState): AutomationPolicyState {
...
export class AutomationPolicyRepository {
  ...
  constructor(persistence: AutomationPolicyPersistence) {
