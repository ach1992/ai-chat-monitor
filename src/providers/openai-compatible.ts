...
{"decision":"CONTINUE|HOLD|UNSURE","reasonCode":"HUMAN_APPROVAL_REQUIRED|MATERIAL_DECISION_REQUIRED|HUMAN_OPERATION_REQUIRED|PROJECT_COMPLETE|USER_STOP|STAGNATION|PLATFORM_ERROR|RATE_LIMIT|SAFETY_BOUNDARY|NEEDLESS_TURN_BOUNDARY|AMBIGUOUS|OTHER","reason":"<=240 chars","confidence":0..1}
Use only the reasonCode values listed in that schema. Never invent aliases or near-synonyms.
...
HOLD is required when continued automation should wait for a human or when the requested outcome is complete. Use the most specific valid reasonCode:
...
- a material choice or decision, including asking the user to choose among material alternatives => MATERIAL_DECISION_REQUIRED;
- explicit user stop => USER_STOP;
- safety boundary => SAFETY_BOUNDARY;
- platform/error boundary => PLATFORM_ERROR;
- provider/platform rate-limit boundary => RATE_LIMIT;
- repeated non-progress => STAGNATION;
- another definite non-continuation HOLD boundary not covered above => OTHER.
...
UNSURE is required with reasonCode AMBIGUOUS whenever the bounded context is insufficient or it is not possible to distinguish confidently between completion, a human boundary, and a needless turn boundary. OTHER may be used for UNSURE only when the ambiguity is definite but does not fit another specific code. Never turn uncertainty into CONTINUE and never assume that a speculative "continue if you can" is safe simply because more work might exist.`;
    ...
    if (typeof content !== "string") {
      if (firstChoice?.finish_reason === "length") {
        throw new ProviderFailure(
          "INVALID_RESPONSE",
          "Provider exhausted the classifier output budget before returning text output.",
        );
      }
