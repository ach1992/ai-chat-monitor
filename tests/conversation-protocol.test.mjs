...
test("the one-time protocol prompt is readable, bounded, and explains future replies", () => {
  assert.match(DEFAULT_CONVERSATION_PROTOCOL_PROMPT, /^\[Chat Turn Guardian — one-time conversation protocol\]\n/);
  assert.match(DEFAULT_CONVERSATION_PROTOCOL_PROMPT, /ACTIVATION\n/);
  assert.match(DEFAULT_CONVERSATION_PROTOCOL_PROMPT, /FUTURE REPLIES\n/);
  assert.match(DEFAULT_CONVERSATION_PROTOCOL_PROMPT, /STATUS RECORD\n/);
  assert.match(DEFAULT_CONVERSATION_PROTOCOL_PROMPT, /remember this protocol for this conversation/i);
  assert.match(DEFAULT_CONVERSATION_PROTOCOL_PROMPT, /no text after it/i);
